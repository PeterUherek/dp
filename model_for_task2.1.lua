...
--local inputSize = 38
local inputSize = 567       -- the number features in the csv file to use as inputs 
...
--criterion = nn.SequencerCriterion(nn.DistKLDivCriterion())              -- if you want a regression, use ms--criterion = nn.MultiLabelMarginCriterion()       -- potential alt for LogSoftMax / ClassNLLCriterion, but you pass it 1d weights (??)
--criterion = nn.SequencerCriterion(nn.MarginCriterion())
criterion = nn.SequencerCriterion(nn.ClassNLLCriterion())

--Data for train error

local path_to_model = "models/task2/model19.1.forget.dat"
local model_to_save = "models/task2/model19.1.forget.dat"
local write_method = "w"
local path_to_validation_data = "data_for_task2/train_dataY10.all.txt"
local path_to_data = "data_for_task2/train_dataY7.txt"
...
local file_for_train_error = io.open("results_files/task2/train_error4.txt", write_method)
local file_for_test_error = io.open("results_files/task2/test_error4.txt", write_method)
...
function build_data(row)
 ...
 local data = torch.Tensor(VecTfidfSize) 
  
  for i=1,VecTfidfSize do
    data[i] = tonumber(row[i+17]) -- Tfidf value
  end
  ...
  local mask_data = substract_section_for_device(row[17])
  for i=0,8 do
    data[inputSize-(14+i)] = mask_data[i+1] 
  end  

  data[inputSize-13] = get_statement(row[16])
